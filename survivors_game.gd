...
func load_chunk(x, y):
    ...
    for _x in range(chunk_size):
        for _y in range(chunk_size):
            ...
            if tile_map_layer.get_cell_source_id(current_tile_position) == -1:
                tile_map_layer.set_cell(current_tile_position, 0, atlas_position, 0)
    ...
    if !object_placed_range.has_point(tile_map_layer.get_used_rect().position) or !object_placed_range.has_point(tile_map_layer.get_used_rect().end):
        object_placed_range = object_placed_range.merge(tile_map_layer.get_used_rect())
        
    for t in object_tiles_position:
        match t:
            "0":
                for tp in range(object_tiles_position[t].size()):
                    if tile_map_layer.get_cell_source_id(Vector2i(object_tiles_position[t][tp].x, object_tiles_position[t][tp].y)) != -1 or tile_map_layer.get_cell_source_id(Vector2i(object_tiles_position[t][tp].x, object_tiles_position[t][tp].y+1)) == -1:
                        draw_tree(object_tiles_position[t][tp].x, object_tiles_position[t][tp].y)
            "1":
                for tp in range(object_tiles_position[t].size()):
                    if tile_map_layer.get_cell_source_id(Vector2i(object_tiles_position[t][tp].x, object_tiles_position[t][tp].y)) != -1:
        ...
        match ot:
            "0":
                for op in range(other_tiles_position[ot].size()):
                    if tile_map_layer.get_cell_source_id(other_tiles_position[ot][op]) != -1:
                        tile_map_layer.set_cell(other_tiles_position[ot][op], 2, Vector2i(3, 2), 0)

func _physics_process(_delta: float) -> void:
    
    # Chunk loading / unloading
    var world_size = tile_map_layer.get_used_rect() as Rect2i
...
func draw_tree(x, y):
    if tile_map_layer.get_cell_source_id(Vector2i(x, y-1)) != -1:
        tile_map_layer.set_cell(Vector2i(x, y-1), 0, Vector2i(0, 0), 0)
    if tile_map_layer.get_cell_source_id(Vector2i(x, y)) != -1:
        tile_map_layer.set_cell(Vector2i(x, y), 0, Vector2i(0, 1), 0)
    
    if tile_map_layer.get_cell_atlas_coords(Vector2i(x, y-2)) == Vector2i(0, 0):
        tile_map_layer.erase_cell(Vector2i(x, y-2))

func draw_bush(x, y):
    if tile_map_layer.get_cell_source_id(Vector2i(x, y)) != -1 and tile_map_layer.get_cell_source_id(Vector2i(x, y)) == -1:
        tile_map_layer.set_cell(Vector2i(x, y), 0, Vector2i(0, 2), 0)
